...
- この規範は地の文（説明・論証の本文）に適用する。次のものは対象外とし、規範を機械的に当てない。
  - キャッチコピー、スローガン、CTA、規範外のコピー文。煽り、体言止め、感嘆符、対句を意図的に使ってよい。
  - 引用、ログ、コード、書誌情報、識別子（製品名やスラッシュ区切りの規格名など）。
  - 表のセル。簡潔さを優先し、中黒や体言止めを許す（並列が長くなるなら読点に開く）。
- 文書の性格が販促・宣伝寄りのとき（コンセプト資料、ピッチ）は、論証系の規範（パラグラフライティング、反論処理）は緩め、整形と用字（一文一行、中黒・ダッシュ、LLM 口調の排除）を優先して当てる。

## 最小チェックリスト

軽い修正でも必ず見る五点。詳細は以降の節を参照する。

1. 一文ごとに改行しているか。
2. ダッシュ（——／—／―）を地の文・見出しに使っていないか。
3. 中黒（・）を並列に使っていないか（固有名詞内部を除く）。
4. 「重要なのは」「正面から」「掘り下げる」などの LLM 口調を使っていないか。
5. 見出しが、扱う対象か読者の問いを指しているか（手順名・結論セリフでないか）。
...
- 中黒（・）を日本語の並列で使わない。ただし単一の固有名詞の内部では使ってよい。見出しやカテゴリラベルの並列（「日常・ライフスタイル」など）も読点か助詞に開く（「日常とライフスタイル」）。表のセル内は簡潔さを優先して中黒を許すが、要素が三つ以上に増えるなら読点に開く。
...
- 例の節の中でも、**その節の問い・帰結に関係しない**過剰な詳細だけを omit する。議論に必要な具体は残す。省略の典型は、エージェント報告の装飾的精度（時刻、HTTP ステータス、カバレッジ率など）や、後で参照しない固有名など。
...
書き上げたら、次の検索で機械的な違反の残骸を洗う（コードブロック・識別子・製品名の一致は除外して判断する）。

- 並列の中黒とダッシュ：`・` `——` `—` `―`
- 範囲やつなぎに紛れ込んだ波ダッシュ：`〜`（範囲表現以外で地の文に出ていないか）
- LLM 口調：`重要なのは` `正面から` `掘り下げ` `深掘り` `多角的` `包括的` `不可欠` `〜に他ならない`
- 予告・総括の枕：`本章では` `ここでは` `まとめると` `要するに`（直前の言い換えでないか）

一致した箇所が、例外（コード、引用、識別子、キャッチコピー）に当たるかを一件ずつ確かめ、地の文の違反だけを直す。

## 出典

本スキルは k16shikano 氏の gist（https://gist.github.com/k16shikano/fd287c3133457c4fd8f5601d34aa817d ）を基に、適用範囲と例外、最小チェックリスト、中黒の見出し・表に関する規定、推敲後セルフチェックなどの節を追記・改変したもの。原典のライセンスは The Unlicense。
