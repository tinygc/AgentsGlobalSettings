...
description: agent 向けテキスト指示（skill / スラッシュコマンド / task プロンプト / instructions / コード生成プロンプト）を、バイアスを排した実行者に動かしてもらい、両面（実行者の自己申告 + 指示側メトリクス）で評価して反復改善する手法。改善が頭 打ちになるまで回す。プロンプトや skill を新規作成・大幅改訂した直後、またはエージェントの挙動が期待通りにならない原因を指示側の曖昧さに求めたいときに使う。
---
...
- skill / スラッシュコマンド / タスクプロンプトを新規作成・大幅改訂した直後
...
-1. **前提チェック — Skill の可用性確認**（静的、dispatch 不要）
   - 対象 skill が `.github/copilot-instructions.md` の `カスタムスキル` セクションに登録されているか確認する
   - パスが正しく、`read_file` で実際に読めるか確認する
   - 未登録であれば `copilot-instructions.md` に追記してから次へ進む
   - **なぜ重要か**: ファイルとして存在しても `copilot-instructions.md` に参照がなければ agent はそのスキルの存在を認識できない。可用性のない skill を empirical test しても意味がない。
...
2. **バイアス排除読み**: 指示を「白紙」の実行者に読ませる。`runSubagent` ツールなどで **新規 subagent を dispatch（起動）** する。自己再読で済ませない（直前に書いた文章を客観視することは構造的に不可能）。並列で複数シナリオを同時実行する場合は単一メッセージ内で複数 Agent 呼び出しを並べる。dispatch 不能環境の扱いは「環境制約」節を参照。
...
4. **両面評価**: 戻ってきた結果から次を記録する。
   ...
   - **指示側の計測**（判定規則は本節で一元定義、他箇所は本節を参照する）:
     ...
     - 生々しい結果（`subagent` からの返答）から次の点を観測する。
     - ステップ数（ツールの使用回数、処理のやり取り数など。Read / Grep も含める、除外しない）
     - 所要時間（やり取りや処理にかかった時間）
...
| 所要時間 | 実行者の処理時間・回答までの体感時間 | 認知負荷の代替指標 |
...
### `tool_uses` (ツールの使用回数) の質的解釈

精度だけ見ると skill の問題が隠れる。ツールの使用回数を **シナリオ間の相対値** とし て使うと構造的欠陥が見える:

- シナリオ間で他シナリオ比 **3-5 倍以上** なら、その skill は **decision-tree index 寄りで自己完結性が低い** サイン。実行者が references descent を強いられてい る
- 典型例: 全シナリオのツール使用回数が 1-3 なのに 1 シナリオだけ 15+ → そのシナリオ用の recipe が skill 内に無く、ファイルやリファレンスを横断探索している
- 対処: iter 2 で「最小完成例 inline」や「いつ references を読むかの指針」を SKILL.md 冒頭に追加するとツールの使用回数は大幅低下する

精度 100% でもツールの使用回数に偏りがあれば iter 2 発動の根拠になる。「精度のみで 判断して打ち切り」は構造的欠陥を見逃しがち。
...
呼び出し側はレポートから自己申告部分を抽出し、使用したツール数や処理時間を評価軸表として埋める。

## 環境制約

新規 subagent を dispatch できない環境（既に subagent として動作している等）では、本 skill は **適用しない**。
- 代替案 1: 新しい GitHub Copilot Chat セッションを起動して改めて依頼する
